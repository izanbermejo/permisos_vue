<template>
    <Dialog v-model:visible="visible" modal @hide="hide"
        :header="isEditMode ? $t('historicMoviments.titolModalModificarFerralla') : $t('historicMoviments.titolModalFerralla')"
        :closable="true" :closeOnEscape="true" :draggable="false"
        style="width: 475px">
        <div class="form">
            <div class="element-form">
                <label>{{ $t('historicMoviments.ferrallaData') }}</label>
            ...
            <div class="element-form">
                <label>{{ $t('historicMoviments.ferrallaQuantitat') }}</label>
            ...
            <div class="break" />
            <div class="element-form">
                <label>{{ $t('historicMoviments.ferrallaObservacions') }}</label>
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const desar = async () => {
            ...
            if (!valid) return;
            if (isEditMode.value) {
                await carrega(InventariService.modificarFerralla(
                    props.movimentId, state.data, state.quantitat,
                    state.observacions || undefined
                ));
                toast.add({ severity: 'success', summary: t('historicMoviments.ferrallaModificadaOk'), life: 3000 });
            } else {
                await carrega(InventariService.afegirFerralla(
                    props.artint, props.clicod, props.empresa, props.magatzem,
                    state.data, state.quantitat,
                    state.observacions || undefined
                ));
                toast.add({ severity: 'success', summary: t('historicMoviments.ferrallaOk'), life: 3000 });
            }
            emit('saved');
            hide();
        };
