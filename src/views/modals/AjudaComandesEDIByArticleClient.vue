  ...
  <Dialog v-model:visible="visible" modal @hide="hide" :closable="true" :closeOnEscape="false" :header="' '"
    ...
    <template #header>
      <div style="font-size: larger; font-weight: 600; float: left;">
        ...
        <span style="margin-left: 10px;"> {{ `${$t('Comandes.comandes del articleclient')} ${refArticleClient}` }}</span>
      </div>
    </template>
    <GridComandesEDIByArticleClient ref="gridComandesArticleClient" :modalParentObert="modalVisible" style="margin-top: 10px;" @mostraTXT="mostrarContingutTXT($event)" @mostraPDF="descarregarPDF($event)" @pujaPDF="openFileInput($event)" @update:close="hide"/>
  </Dialog>
...
import GridComandesEDIByArticleClient from "@/components/edi/GridComandesEDIByArticleClient.vue";
import EditorComentaris from '@/views/modals/EditorComentaris.vue';
import Swal from "sweetalert2";
import { useI18n } from "vue-i18n";
  
export default {
  name: "AjudaComandesEDIByArticleClient",
  components: {
    GridComandesEDIByArticleClient,
    EditorComentaris,
  },
  props: {
    artCli : String,
  ...
  setup(props, { emit }) {
    ...
    const gridComandesArticleClient = ref();
    const refArticleClient = toRef(props, 'artCli');
    const comandesArticleClient = ref();
    ...
    const buscar = async () => {
      comandesArticleClient.value = await carrega(ediService.obtenirComandesEdiByArticleClient(refArticleClient.value));
      gridComandesArticleClient.value.performSearch(comandesArticleClient.value);
    ...
    return {
      ...
      visible,
      hide,
      gridComandesArticleClient,
      refArticleClient,
      comandesArticleClient,
      articleClient,
