  ...
  <Dialog v-model:visible="visible" :header="$t('ModalAdjuntsComandes.Titol')" @hide="hide()" modal
    ...
    :closeOnEscape="false">
      <div v-if="props.potEditar" class="file-drop-area">
        ...
        <span class="file-msg">{{ $t('ModalAdjuntsAlbarans.Instruccions pujar') }}</span>
      ...
      <DataTable :value="adjunts" class="p-datatable-sm" 
          ...
          <template #empty>
              {{ $t('ModalAdjuntsAlbarans.Cap adjunt') }}
          </template>
          ...
          <Column :header="$t('ModalAdjuntsAlbarans.Fitxer')" :style="{width:'350px'}" >
            <template #body="{data}">
              <a class="enllas" @click="descarregarFitxer(data)">{{ data.nom }}</a>
            </template>
          </Column>
          <Column :header="$t('ModalAdjuntsAlbarans.Data')" :style="{width:'140px'}" bodyStyle="justify-content: right;" >
            <template #body="{data}">
              {{ $d(data.data, 'long') }}
          </template>
          </Column>
          <Column :header="$t('ModalAdjuntsAlbarans.Usuari')" :style="{width:'230px'}" >
            <template #body="{data}">
...
import AlbaransService from '@/services/albara.service';
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const menuModel = ref([
      {label: ()=>  t('ModalAdjuntsAlbarans.Descarregar'), class:'p-button-text', icon: 'pi pi-download', action: () => clickOpcioDescarregar()},
      {label: ()=>  t('ModalAdjuntsAlbarans.Canviar nom'), class:'p-button-text', icon: 'pi pi-pencil', action: () => visibleModalRenombrar.value = true, hide: () => !props.potEditar},
      {label: ()=>  t('ModalAdjuntsAlbarans.Eliminar'), class:'p-button-text p-button-danger', icon: 'pi pi-trash', action: () => clickOpcioEliminar(), hide: () => !props.potEditar},
    ]);

    onMounted(async () => {
      try {
        adjunts.value = await carrega(AlbaransService.obtenirAdjunts(props.albara));
    ...
    const noPermisos = async () => {
      await Swal.fire({
        ...
        icon: 'error',
        title: t('App.Atencio'),
        text: t('ModalAdjuntsAlbarans.No pot pujar fitxers'),
      });
    };
    ...
    const clickOpcioEliminar = async () => {
      confirm.require({
        ...
        message: t('ModalAdjuntsAlbarans.Confirma Elimina Adjunt'),
    ...
    const eliminarFitxer = async () => {
      await carrega(AlbaransService.eliminarAdjunt(props.albara, fitxer.value.codiFitxer));
      adjunts.value = await carrega(AlbaransService.obtenirAdjunts(props.albara));
      toast.add({severity:'success', summary: t('ModalAdjuntsAlbarans.Fitxer eliminat OK'), life: 3000});
      emit('update:numAdjunts', adjunts.value?.length);
    }

    const pujarFitxersServidor = async (files) => {
      let filesUploads = files.map(f => AlbaransService.pujarAdjunt(props.albara, f)
        .then((reqResult) => {
          return {
            source: f.name,
            value: reqResult
      ...
      if (fitxersAmbError.length) {
        ...
        Swal.fire({
            icon: 'error',
            title: t('ModalAdjuntsAlbarans.Error Pujant Fitxers'),
            html: missatge
        });
      }
      if (fitxersOk.length) {
        fitxersOk.forEach(item => {
          ...
          toast.add({severity:'success', summary: t('ModalAdjuntsAlbarans.Fitxer pujat OK', [nomFitxer]), life: 3000});
        })
    ...
    const descarregarFitxer = async (dataFitxer) => {
      let resp = await carrega(AlbaransService.descarregarAdjunt(props.albara, dataFitxer.codiFitxer));
    ...
    const renombrarAdjunt = async (nouNomFitxer) => {
      await carrega(AlbaransService.renombrarAdjunt(props.albara, fitxer.value.codiFitxer, nouNomFitxer));
      adjunts.value = await carrega(AlbaransService.obtenirAdjunts(props.albara));
