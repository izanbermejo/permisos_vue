<template>
  <Dialog v-model:visible="visible" modal @hide="hide" :closable="true" :closeOnEscape="true" draggable
    style="width: 700px">
    <template #header>
      <div style="width: 100%;">
    ...
    <div class="form" style="margin-top: 5px;">
      <div class="element-form">
        <label>{{$t('Comandes.Format distribuidor')}}:</label>
          <span>
            <SelectButton v-model="state.isFormatDistribuidor" :options="opcionsSiNo" optionValue="clau" optionLabel="valor" />
          </span>
      </div>
      ...
      <div class="element-form">
        <span>
          ...
          <a style="cursor: pointer; color: #2196f3; text-decoration: underline;" @click="missatgeMinim">
            {{ `(${$t('Comandes.Enganxa missatge import minim')})` }}
          </a> 
        </span>
          <span>
...
setup(props, { emit }) {
  ...
  const state = reactive({
    ...
    cc: email(),
    assumpte: `Order: ${refComanda.value.comandaClient}`,
    missatge: '',
    idioma : props.client.idioma,
    isFormatDistribuidor : props.client.isDistribuidor,
    formatNumeric : '',
  });
  ...
  const missatgeEstandard = () => {
    ...
    switch(state.idioma) {
      case 'ca': missatge = MissatgesContants.MISSATGE_STANDARD_CATALA; 
                 missatgeDesc = MissatgesContants.MISSATGE_STANDARD_CATALA_DESC;
                 break;
      case 'es': missatge = MissatgesContants.MISSATGE_STANDARD_CASTELLA; 
                 missatgeDesc = MissatgesContants.MISSATGE_STANDARD_CASTELLA_DESC;
                 break;
      case 'de': missatge = MissatgesContants.MISSATGE_STANDARD_ALEMANY; 
                 missatgeDesc = MissatgesContants.MISSATGE_STANDARD_ALEMANY_DESC;
                 break;
      case 'fr': missatge = MissatgesContants.MISSATGE_STANDARD_FRANCES; 
                 missatgeDesc = MissatgesContants.MISSATGE_STANDARD_FRANCES_DESC;
                 break;
      case 'it': missatge = MissatgesContants.MISSATGE_STANDARD_ITALIA 
                 missatgeDesc = MissatgesContants.MISSATGE_STANDARD_ITALIA_DESC;
                 break;
      default  : missatge = MissatgesContants.MISSATGE_STANDARD_ANGLES; 
                 missatgeDesc = MissatgesContants.MISSATGE_STANDARD_ANGLES_DESC;
                 break;
    }
    state.missatge = state.isFormatDistribuidor ? missatge.replace('{descompte}', missatgeDesc+ '\n') : missatge.replace('{descompte}', '');
  }

  const missatgeMinim = () => {
    switch(state.idioma) {
      case 'ca': state.missatge = MissatgesContants.MISSATGE_MINIM_CATALA; 
                 break;
      case 'es': state.missatge = MissatgesContants.MISSATGE_MINIM_CASTELLA; 
                 break;
      case 'de': state.missatge = MissatgesContants.MISSATGE_MINIM_ALEMANY; 
                 break;
      case 'fr': state.missatge = MissatgesContants.MISSATGE_MINIM_FRANCES; 
                 break;
      case 'it': state.missatge = MissatgesContants.MISSATGE_MINIM_ITALIA; 
                 break;
      default  : state.missatge = MissatgesContants.MISSATGE_MINIM_ANGLES; 
                 break;
    }
  }
  ...
  const enviaJustificant = async () => {
    ...
    if (!v.value.$error) {
      await(carrega(ComandesService.enviarJustificantNormalitzat(refComanda.value.comanda, state, fileList.value)));
      hide();
  ...
  const previsualitzarJustificant = async () => {
    let resp = await carrega(ComandesService.generarJustificantNormalitzat(refComanda.value.comanda, state.idioma, state.formatNumeric, state.isFormatDistribuidor));
    obrirPdfNovaPestanya(resp, `${t('Comandes.Justificant')}_${refComanda.value.comanda}_${state.idioma}`);
  }
