<template>
  ...
  <ModalAfegirEmpleatDelParametre v-if="visibleAfegirEmpleatDelParametre" :carregat="visibleAfegirEmpleatDelParametre"
    @update:carregat="visibleAfegirEmpleatDelParametre = $event" :nomAplicacio="props.nomAplicacio"
    :nomParametre="props.nomParametre" @actualitzar="carregaEmpleatsDelParametre()"/>
  <Dialog v-model:visible="visible" modal @hide="hide" :closable="true" :closeOnEscape="false" :draggable="true" :contentStyle="{ overflowY: 'hidden' }">
    <template #header>
      <div style="width: 100%;">
        <span style="font-size: larger; font-weight: 600; float: left;">{{ `${$t('Parametres.empleat parametre')} ${props.nomAplicacio} - ${props.nomParametre}` }}</span>
        <ButtonShortcut :literal="$t('Parametres.Afegir empleat del Parametre')" @click="visibleAfegirEmpleatDelParametre = true"
    ...
    :sortField="sortField" :sortOrder="sortOrder" @sort="onSort">
      ...
      <Column :header="$t('App.Nom')" :style="{ width: '350px' }" style="max-width: 250px;" field="nom">
        <template #body="{ data }">
          {{ data.nom }}
        </template>
      </Column>
      <Column :header="$t('App.Cognoms')" :style="{ width: '350px' }" style="max-width: 300px;" field="cognoms">
        <template #body="{ data }">
          {{ data.cognoms }}
        </template>
      </Column>
      ...
      <Column :header="$t('App.Valor')" :style="{ width: '350px' }"  field="valor">
        <template #body="{ data }">
          {{ data.valor }}
        </template>
      </Column>
...
import ModalAfegirEmpleatDelParametre from './ModalAfegirEmpleatDelParametre.vue';

export default {
  name: 'ModalEmpleatsDelParametre',
  components: {
    ModalFuncionsEmpleat,
    ModalAfegirEmpleatDelParametre,
  },
  props: {
    nomAplicacio: {
      type: String,
  ...
  setup(props, { emit }) {
    ...
    const visibleAfegirEmpleatDelParametre = ref(false);
    ...
    onMounted(async () => {
      ...
      await carregaEmpleatsDelParametre();
    });
    ...
    const modalVisible = computed(() => {
      ...
      || visibleAfegirEmpleatDelParametre.value;
    });
    ...
    const carregaEmpleatsDelParametre = async () => {
      const data = await parametresService.obtenirEmpleatsByParametre(props.nomAplicacio, props.nomParametre);
      empleats.value = data;
    }
    ...
    const menuModel = computed(() => {
        ...
        result.push({ label: () => `${t('Empleats.eliminar empleat')}`, class: 'p-button-text', icon: 'pi pi-trash', command: () => eliminarEmpleat(props.nomAplicacio, props.nomParametre) });
      return result;
    });
    ...
    return {
      ...
      visibleAfegirEmpleatDelParametre,
      props,
      onSort,
      empleats,
      registreSeleccionat,
