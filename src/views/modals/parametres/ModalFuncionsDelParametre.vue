<template>
  ...
  <ModalAfegirFuncioDelParametre v-if="visibleAfegirFuncioDelParametre" :carregat="visibleAfegirFuncioDelParametre"
    @update:carregat="visibleAfegirFuncioDelParametre = $event" :nomAplicacio="props.nomAplicacio"
    :nomParametre="props.nomParametre" @actualitzar="carregaFuncionsDelParametre()"/>
  <Dialog v-model:visible="visible" modal @hide="hide" :closable="true" :closeOnEscape="false" :draggable="true" :contentStyle="{ overflowY: 'hidden' }">
    <template #header>
      <div style="width: 100%;">
        <span style="font-size: larger; font-weight: 600; float: left;">{{ `${$t('Parametres.funcionsParametre')}: ${props.nomAplicacio} - ${props.nomParametre}` }}</span>
        <ButtonShortcut :literal="$t('Moduls.Afegir Funcio del Modul')" @click="visibleAfegirFuncioDelParametre = true"
    ...
    :sortField="sortField" :sortOrder="sortOrder" @sort="onSort">
      ...
      <Column :header="$t('App.Centre')" :style="{ width: '350px' }" style="max-width: 300px;" field="centre">
        <template #body="{ data }">
          <span v-if="data.nomCentre"> {{ data.nomCentre }} </span>
          <span v-else><b>***</b></span>
        </template>
      </Column>
      <Column :header="$t('App.Departament')" :style="{ width: '350px' }" style="max-width: 300px;" field="departament">
        <template #body="{ data }">
      ...
      <Column :header="$t('App.Valor')" :style="{ width: '350px' }"  field="funcio">
        <template #body="{ data }">
          {{ data.valor }}
        </template>
      </Column>
...
import ModalAfegirFuncioDelParametre from './ModalAfegirFuncioDelParametre.vue';
import { useConfirm } from 'primevue/useconfirm';
import { isContextMenuKey } from '@/utils/contextmenuUtils.js';

export default {
  name: 'ModalFuncionsDelParametre',
  components: {
    ModalEmpleatsCentDepFun,
    ModalAfegirFuncioDelParametre,
  },
  props: {
    nomAplicacio: {
      type: String,
  ...
  setup(props, { emit }) {
    ...
    const visibleAfegirFuncioDelParametre = ref(false);
    ...
    onMounted(async () => {
      ...
      await carregaFuncionsDelParametre();
    });
    ...
    const modalVisible = computed(() => {
      ...
      || visibleAfegirFuncioDelParametre.value;
    });
    ...
    const carregaFuncionsDelParametre = async () => {
      const data = await parametresService.obtenirFuncionsByParametre(props.nomAplicacio, props.nomParametre);
      funcions.value = data;
    }
    ...
    const eliminarFuncio = async (nomAplicacio, nomParametre) => {
      ...
      confirm.require({
        ...
        accept: async () => {
          let request = {
            ...
            valor: registreSeleccionat.value.valor,
          }

          await parametresService.eliminarFuncioDelParametre(nomAplicacio, nomParametre, request);
          carregaFuncionsDelParametre();
        }
      });
    };
    ...
    const menuModel = computed(() => {
      ...
      result.push({ label: () => `${t('Funcions.eliminar funcio')}`, class: 'p-button-text', icon: 'pi pi-trash', command: () => eliminarFuncio(props.nomAplicacio, props.nomParametre) });
      return result;
    });
    ...
    return {
      ...
      visibleAfegirFuncioDelParametre,
      props,
      onSort,
      funcions,
      registreSeleccionat,
