<template>
    ...
    <div id="ajudaArticlesAltres" class="card-body bodyfocused" style="min-height: 27vh; max-height: 27vh;">
    <DataTable :value="articles" class="p-datatable-sm-petita"
        ...
        <Column :header="$t('Comandes.Preu')" :style="{width:'150px'}" style="max-width: 150px;">
            <template #body="{data}">
                <div style="width: 100%; text-align: right;">
                    <span v-if="data.tipus != 'IBINSA'">
        ...
        <Column :header="$t('Comandes.Embalatge')" :style="{width:'210px'}" style="text-align: right; max-width: 210px;">
            <template #body="{data}">
                <div style="width: 100%; text-align: right;">
                {{ `${$n(data.unitatsEmbalatge)} x ${$n(data.caixesPalet)} (${$n(data.unitatsEmbalatge*data.caixesPalet)})` }}
                </div>
            </template>
        </Column>
        </DataTable>
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const elementClient = () => document.getElementById('ajudaArticlesAltres');
        ...
        const performSearch = debounce(async() => {
            ...
            if (!props.caractersMinimFiltre || props.filtre.length >= 3) {
                articles.value = await ComandesService. queryArticles(props.client, props.tipus, props.filtre, abortController);
        ...
        const menuModel = ref([
            {label: ()=>  `${t('AjudaArticlesNormalitzats.Veure stock')} [Ctrl+s]`, class:'p-button-text', icon: 'pi pi-box', command: () => emit('showStocks', selectedArticle.value)},
            {label: ()=>  `${t('ModalAjudaArticleClient.Trasabilitat')}`, class:'p-button-text', icon: 'pi pi-list', command: async () => {
                await descarregaTrasabilitat();
            }},
            {label: ()=>  `${t('ModalAjudaArticleClient.Localitzacio')}`, class:'p-button-text', icon: 'pi pi-map-marker', command: async () => {
        ...
        const descarregaTrasabilitat = async () => {
            ...
            saveAs(new Blob([resp]), `${ t('ModalAjudaArticleClient.Trasabilitat') + selectedArticle.value.clicod}.xls`);
        }
