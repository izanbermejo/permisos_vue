<template>
  <Dialog v-model:visible="visible" modal @hide="hide" :closable="true" :closeOnEscape="false"
    :draggable="true" :header="$t('Permisos.Afegir Empleat al permis')" style="width: 500px">
...
export default {
  name: "ModalAfegirEmpleatDelPermis",
  props: {
    nomAplicacio: {
      type: String,
      default: null
  ...
  setup(props, { emit }) {
    ...
    const guardar = async () => {
      ...
      await carrega(PermisosService.assignarEmpleat( props.nomAplicacio, props.nomModul, props.nomPermis, empleat.value));
