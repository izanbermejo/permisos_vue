<template>
  <Dialog v-model:visible="visible" modal @hide="hide" :closable="true" :closeOnEscape="true" :draggable="true"
    ...
    <template #header>
      <div style="width: 100%;">
        <span style="font-size: larger; font-weight: 600; float: left;">{{ $t('Comandes.Exportar detall comanda') }}</span>
      </div>
    </template>
    ...
    <div style="display: flex; justify-content: flex-end; margin-top: 10px;">
      <Button @click="exportarDetallsClient" :label="$t('Comandes.Descarregar taula')" icon="pi pi-file-excel" class="p-button-sm" style="margin-left: 10px;"/>
    </div>
  </Dialog>
</template>
...
export default {
name: 'ModalExportarDetallComanda',
props: {
  comanda: Number,
},
setup(props, { emit }) {
  ...
  const exportarDetallsClient = async () => {
    ...
    const resp = await carrega(ComandesService.exportarDetallComanda(state.comanda, params));

    saveAs(new Blob([resp]), `DetallComanda_${state.comanda}.xlsx`);
