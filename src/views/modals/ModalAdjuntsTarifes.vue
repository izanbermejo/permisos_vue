<template>
  ...
  <Dialog v-model:visible="visible" :header="$t('ModalAdjuntsTarifes.Titol')" @hide="hide()" modal
...
export default {
  ...
  setup(props, { emit }) {
    ...
    onMounted(async () => {
      console.log("carregat: " + props.carregat);
    ...
    const pujarFitxersServidor = async (files) => {
      let filesUploads = files.map(f => TarifesService.altaTarifa(f)
        .then((reqResult) => {
          return {
            source: f.name,
            value: reqResult
