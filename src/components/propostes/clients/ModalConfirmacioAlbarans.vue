<template>
  ...
  <Dialog :visible="visible" modal :closable="true" :closeOnEscape="false" :draggable="false"
    ...
    <div class="control-aprofitar">
      <Checkbox v-model="aprofitar" :binary="true" inputId="chkAprofitarOberts" @change="emitRecalcular" />
      <label for="chkAprofitarOberts">{{ $t('Propostes.Aprofitar albarans oberts') }}</label>
    </div>
    ...
    <div v-if="albaransNous.length" class="seccio">
      ...
      <div v-for="(albara, idx) in albaransNous" :key="`nou-${idx}`" class="card albara-card">
        <div class="card-header albara-header">
          ...
          <span v-if="albara.autofacturable" class="badge-autofac badge-autofac-ok">
            <font-awesome-icon icon="fa-solid fa-circle-check" style="margin-right: 5px;" />
            {{ $t('Albarans.Autofacturable') }}
          </span>
          <span v-else class="badge-autofac badge-autofac-no">
            <font-awesome-icon icon="fa-solid fa-circle-xmark" style="margin-right: 5px;" />
            {{ $t('Albarans.No autofacturable') }} — {{ motiuAutofacturable(albara.motiuNoAutofacturable) }}
          </span>
        </div>
        <DataTable :value="albara.linies" class="p-datatable-sm taula-linies" showGridlines>
          <Column :header="$t('Propostes.Article')" headerStyle="width:95px" bodyStyle="width:95px">
            <template #body="{ data }"><div class="cel-ellipsis" v-tooltip="data.article">{{ data.article }}</div></template>
          </Column>
          ...
          <Column :header="$t('Propostes.Comanda client')" headerStyle="width:130px" bodyStyle="width:130px">
            <template #body="{ data }"><div class="cel-ellipsis" v-tooltip="data.comandaClient">{{ data.comandaClient }}</div></template>
          </Column>
          <Column :header="$t('Propostes.Q pendent')" headerStyle="width:85px" bodyStyle="width:85px; text-align:right">
            <template #body="{ data }">{{ $n(data.quantitat) }}</template>
          </Column>
          <Column :header="$t('Propostes.Preu')" headerStyle="width:115px" bodyStyle="width:115px; text-align:right">
            <template #body="{ data }">{{ `${$n(data.preu, 'decimalLong')} ${data.divisa}` }}</template>
          </Column>
          <Column :header="$t('Albarans.Facturacio')" headerStyle="width:85px" bodyStyle="width:85px; text-align:center">
            <template #body="{ data }">
              <span v-if="data.preuFixat" v-tooltip="$t('Comandes.Preu fixat')" class="marca-linia">
                <font-awesome-icon icon="fa-solid fa-hand" />
              </span>
              <span v-if="data.comandaBlanca" v-tooltip="$t('Albarans.Comanda blanca')" class="marca-linia">
          ...
          <Column headerStyle="width:130px" bodyStyle="width:130px; text-align:right">
            ...
            <template #body="{ data }"><span v-tooltip="embalatgeTooltip(data)">{{ embalatgeInfo(data) }}</span></template>
          </Column>
          <Column :header="$t('Albarans.Avis embalatge')" headerStyle="width:230px" bodyStyle="width:230px">
            <template #body="{ data }">
              <span v-if="data.avisEmbalatge" v-tooltip="avisEmbalatgeText(data.avisEmbalatge)" class="avis-emb-cel">
    ...
    <!-- Albarans oberts que s'aprofiten -->
    ...
    <!-- Albarans oberts descartats per l'usuari (es poden tornar a aprofitar) -->
    ...
    <!-- Albarans oberts que NO es poden aprofitar perquè tenen una altra data (informatiu) -->
...
export default {
  ...
  setup(props, { emit }) {
    ...
    // Opcions d'aprofitament d'albarans oberts (Nivell 0: toggle global; Nivell 1: descartar per albarà)
    ...
    const motiuAutofacturable = (codi) => {
      switch (codi) {
        case 'CLIENT_NO_AUTOFACTURABLE': return t('Albarans.Motiu client no autofacturable');
        case 'LINIA_PREU_FIXAT_O_COMANDA_BLANCA': return t('Albarans.Motiu linia preu fixat o comanda blanca');
        default: return '';
      }
    };
    ...
    // Tooltip amb el detall de l'embalatge, igual que a comandes especials
    const embalatgeTooltip = (data) => {
      if (!data.unitatsEmbalatge || !data.caixesPalet) return '';
      return t('Comandes.Embalatge resultat', [n(data.unitatsEmbalatge), data.caixesPalet, n(data.unitatsEmbalatge * data.caixesPalet)]);
    };
    ...
    // Gestió de tecles pròpia del modal: Esc tanca només aquest modal (no el del darrere)
    // i Enter confirma la creació. S'atura la propagació perquè el modal de fons no reaccioni.
    const keyHandler = (ev) => {
    ...
    watch(() => props.visible, (obert) => {
      if (obert) {
        ...
        // Captura per rebre la tecla abans que el listener del modal de fons
    ...
    return {
      ...
      detallVisible, detallIdAlbara, obrirDetallAlbara, motiuAutofacturable, formatData,
      embalatgeInfo, embalatgeTooltip, avisEmbalatgeText, avisEmbalatgeIcon,
      aprofitar, albaransDescartats, emitRecalcular, descartarAlbara, reincorporarAlbara,
    };
  },
}
</script>
