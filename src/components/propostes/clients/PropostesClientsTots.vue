<template>
  <ModalPropostaClient v-if="visibleModalPropostaClient" :carregat="visibleModalPropostaClient"
      ...
      :resumClient="selectedClient" :dataPropostes="dataPropostes" :tipus="'tots'"/>
  <div class="card">
...
import { computed, onActivated, onDeactivated, onMounted, ref, toRef } from 'vue';
...
import ModalPropostaClient from './ModalPropostaClient.vue';
import { watch } from 'vue';
import { useObtaPropostesClient } from '@/composables/usePropostesClient.js';

export default {
  // eslint-disable-next-line  
  name: 'PropostesClientsTots',
  components : {
    ColumnaServible,
    ColumnaTipologia,
    ModalPropostaClient,
  ...
  setup ( props, {emit} ) {
    ...
    const listClients = ref(infoPropostes.value.resumClient);
    ...
    watch(infoPropostes, () => {
      listClients.value = infoPropostes.value.resumClient;
      selectedIndex.value = 0;
    });
