<template>
  <ModalPropostaClientNoFerm v-if="visibleModalPropostaClient" :carregat="visibleModalPropostaClient"
      ...
      :resumClient="selectedClient" :dataPropostes="dataPropostes" :tipus="'invent'"/>
  <div class="card">
    <h5 class="card-header">
      {{ $t('Propostes.Clients') }}
    </h5>
    <div id="elemenGridPropostesClientInvent" style="padding: 10px;">
...
export default {
  ...
  setup ( props, {emit} ) {
    ...
    const listClients = ref(infoPropostes.value.resumClientInvent);
    ...
    const elementGridPropostesClient = () => document.getElementById('elemenGridPropostesClientInvent');
    ...
    watch(infoPropostes, () => {
      listClients.value = infoPropostes.value.resumClientInvent;
    });
