<template>
  <div class="card">
    <h5 class="card-header">
      ...
      <font-awesome-icon icon="fa-solid fa-chart-bar" style="font-size: 1.25rem" />
      {{ $t('Entrades.Part comercial') }}
    </h5>
    <div class="card-body">
      ...
      <div ref="gridWrapper" style="margin-top: 10px;">
        <GridEntradesComercial ref="gridEntradesComercial" :modalParentObert="modalVisible" :focused="focusedGrid" :recarrega="buscar"
          @showDetall="showDetallEntrada" @recarrega="buscar"/>
      </div>
    </div>
  </div>
</template>
...
import GridEntradesComercial from './GridEntradesComercial.vue';
...
export default {
  name: 'EntradesComercial',
  components : {
    GridEntradesComercial,
  },
  setup ( props, {emit} ) {
    const { t } = useI18n();
    const permisosStore = usePermisosStore();
    const gridEntradesComercial = ref();
    ...
    const buscar = async () => {
      ...
      let resultat = await(carrega(entradesService.mostrarEntradesComercial(params)));
      gridEntradesComercial.value.performSearch(resultat);
    ...
    const exportar = () =>{
      gridEntradesComercial.value.exportarTaula();
    };
